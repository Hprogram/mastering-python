...
def kCrossoverLearing():
    ...
    learningResult = np.zeros((totalSize, 2, 9), dtype=np.float64)
    model = getCompiledModel(x_train, y_train)
    init_weights = model.get_weights()
    
    checkedFileName = {}
    i = 0
    normalizer = EQNormalizer.EQNormalizer()
    checkList = []
    # import random
    # random.seed(119)
    ...
    # np.save("./resultDataEQ.npy",learningResult)
...
def drawGraph(data):
    global savePng
    subjects = ["HighPass", "HighShelf", "HighShelfDB", "Bell1", "Bell1DB", "Bell2", "Bell2DB", "Bell3", "Bell3DB"]
    plt.figure(figsize=(20,20), dpi=100)
    percents =[]
    for i, subject in enumerate(subjects):
        if subject == 'Bell3':
            for j , k in enumerate(data[:,0,i]):
                if  k == 1.2: 
                    data[:,0,i][j] = 1200
        plt.subplot(9,1,i+1)
        removeList = []
        for j , k in enumerate(data[:,0,i]):
            if k ==0: removeList.append(j)
        # for j in range(len(removeList), 0, -1):
        #     del data[removeList[j-1]]
        #     # del data[:,1,i][removeList[j-1]]
        data = np.delete(data, removeList,0)
    ...
    savePng = savePng.replace('./ResultEQ/', './ResultEQ/'+str(round(percents[0]+percents[4],1))+"_")
    plt.show()
    plt.savefig(savePng)
    plt.close('all')

    return round(percents[0]+percents[4],1) >= 140


def copyModelWrapper():
    global savePng
    shutil.copy2('LearningModule/ModelWrapperEQ.py', savePng.replace(".png", ".py"))
...
if (__name__ == '__main__'):
    ...
    itemCnt = 0
    for next_nb_layers in range(8,9):
        ...
        for item in product(permu_kernel_size, permu_nb_filters):
            itemCnt += 1
            if itemCnt <= 918: continue
            kernel_size = item[0]
            nb_filters = item[1]
            nb_layers = len(nb_filters)
            savePng = "./ResultEQ/%s_%s_%s_%s.png" %(time.strftime('%m-%d-%H-%M-%S', time.localtime(time.time())), str(nb_filters), str(kernel_size), itemCnt)
            # copyModelWrapper()
            print (savePng)
            kCrossoverLearing()
